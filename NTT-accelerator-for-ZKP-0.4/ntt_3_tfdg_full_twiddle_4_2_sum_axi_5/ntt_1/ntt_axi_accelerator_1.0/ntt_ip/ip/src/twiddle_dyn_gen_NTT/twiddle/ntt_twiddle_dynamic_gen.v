...
modular_multiplier_256bit mod_mult_inst (
    ...
    .N(current_modulus),
    .Np(current_Np),           // 连接Np
    .R2_mod_N(current_R2_mod_N), // 连接R2_mod_N
        ...
        case (state)
            STATE_IDLE: begin
                if (start_gen_rising) begin
                    ...
                    current_Np <= Np;                   // 锁存Np
                    current_R2_mod_N <= R2_mod_N;       // 锁存R2_mod_N
                    ...
                    $display("[%t] NTT: Modulus = %h", $time, modulus);
                    $display("[%t] NTT: Np = %h", $time, Np);
                    $display("[%t] NTT: R2_mod_N = %h", $time, R2_mod_N);
                end
            end
            ...
            STATE_CALC_EXPONENT: begin
                ...
                if (counter == 0) begin
                    ...
                    // mod_pow_exponent <=256'h7;
                    mod_pow_start <= 1;
                    
                    $display("[%t] NTT: Calculating step value", $time);
                    $display("[%t] NTT: base=%h, exponent=%d", $time, current_base_twiddle, current_N >> (current_stage + 1));
                end
                ...
                end else if (counter > 1000) begin  // 增加超时时间，因为大数模幂需要更长时间
                    // 超时保护
                    $display("[%t] NTT WARNING: mod_pow timeout for step value", $time);
            ...
            STATE_CALC_BASE: begin
                ...
                if (counter == 0) begin
                    ...
                    $display("[%t] NTT: Calculating base value", $time);
                    $display("[%t] NTT: base=%h, exponent=%d", $time, current_base_twiddle, target_exponent);
                ...
                end else if (counter > 1000) begin  // 增加超时时间
                    // 超时保护
            ...
            STATE_PARALLEL_INIT: begin
                ...
                if (counter == 0) begin
                    ...
                    $display("[%t] NTT: base_value = %h, step_value = %h", $time, base_value, step_value);
                end
            ...
            STATE_PARALLEL_GEN: begin
                ...
                if (parallel_done) begin
                    ...
                    // 显示所有并行生成的旋转因子
                    for (j = 0; j < PARALLEL_LEVEL; j = j + 1) begin
                        $display("[%t] NTT: parallel_twiddles_array[%d] = %h", $time, j, parallel_twiddles_array[j]);
                    end
                end else if (counter > 500) begin  // 增加超时时间，因为并行单元可能需要多周期
                    // 超时保护
            ...
            STATE_SELECT_OUTPUT: begin
                ...
                if (counter == 0) begin
                    ...
                    if (current_index < PARALLEL_LEVEL) begin
                        ...
                        $display("[%t] NTT: Selecting index %d from parallel array", $time, current_index);
                    end else begin
                        // 如果索引超出并行范围，需要计算正确的值
                        // 这里简化处理：使用模幂计算正确的值
                        // 实际应用中可能需要更复杂的处理
                        $display("[%t] NTT: Index %d exceeds parallel range, using first value", $time, current_index);
            ...
            STATE_DONE: begin
                ...
                $display("[%t] NTT: Total latency = %d cycles", $time, gen_latency);
