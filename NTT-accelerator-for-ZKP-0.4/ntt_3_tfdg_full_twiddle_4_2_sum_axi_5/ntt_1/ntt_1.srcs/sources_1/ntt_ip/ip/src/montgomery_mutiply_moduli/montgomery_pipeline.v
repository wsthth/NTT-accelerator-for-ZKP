...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        if (start) begin
            $display("[DEBUG] Cycle %0d: 开始计算", cycle_count);
            $display("[DEBUG] t = 0x%h... (512位)", t);
        ...
        if (stage3_valid) begin
            $display("[DEBUG] Cycle %0d: Stage3 - 分段乘法完成", cycle_count);
        end
        
        if (stage4_valid) begin
            debug_mN <= stage4_mN;
            $display("[DEBUG] Cycle %0d: Stage4 - mN = 0x%h... (512位)", 
                     cycle_count, stage4_mN);
        end
        
        if (stage5_valid) begin
            debug_sum <= stage5_sum;
            $display("[DEBUG] Cycle %0d: Stage5 - 最终结果 = 0x%h", 
                     cycle_count, mont_result);
            $display("[DEBUG]                    t + mN = 0x%h... (513位)", debug_sum);
            $display("[DEBUG]                    (t + mN) >> %0d = 0x%h", 
                     TOTAL_BITS, stage5_sum[2*TOTAL_BITS-1:TOTAL_BITS]);
        end
        
        if (valid_out) begin
            $display("[DEBUG] Cycle %0d: 计算完成，结果 = 0x%h", 
                     cycle_count, mont_result);
        end
    end
end
