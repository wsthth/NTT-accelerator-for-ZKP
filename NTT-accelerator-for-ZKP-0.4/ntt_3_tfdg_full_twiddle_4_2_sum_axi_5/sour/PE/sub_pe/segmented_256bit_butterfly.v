...
always @(*) begin
    if (add_mode) begin
        // 蒙哥马利域加法：直接相加后取模
        // 注意：我们需要的是 (x0 + montgomery_result) mod modulus
        // 但在蒙哥马利域中，这仍然是正确的
        
        // 计算临时和
        
        // 检查是否需要减一次模数
        if (temp_sum >= {1'b0, modulus}) begin
            mod_add_sub_result = temp_sum - {1'b0, modulus};
        end else begin
            mod_add_sub_result = temp_sum[TOTAL_WIDTH-1:0];
        end
    end else begin
        // 蒙哥马利域减法
        if (x0 >= montgomery_result) begin
