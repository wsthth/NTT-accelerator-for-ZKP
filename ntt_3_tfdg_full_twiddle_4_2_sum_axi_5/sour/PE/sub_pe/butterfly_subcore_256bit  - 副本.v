...
generate
    for (i = 0; i < SEG_COUNT; i = i + 1) begin : gen_segments
        ...
        // 注意：模数需要整个256位用于模约简，但64位乘法可以使用模数的低64位
        assign mod_seg[i] = modulus[SEG_WIDTH-1:0]; // 使用模数的低64位
    end
endgenerate

// ============================================================================
// 分段乘法器阵列（16个64位乘法器并行计算）
// ============================================================================
generate
...
reg [7:0] weight_reg;        // 权重寄存器

// 权重计算组合逻辑
wire [7:0] weight_calc = (accum_i + accum_j) * SEG_WIDTH;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (accum_phase)
            ...
            3'd1: begin
                // 第二阶段：计算当前权重，开始累加
                weight_reg <= weight_calc;  // 计算权重
                
                // 累加当前乘积
                accumulator <= accumulator + (products[accum_i][accum_j] << weight_reg);
